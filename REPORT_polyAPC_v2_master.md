...
**Result — bare and poly approach the electrode *oppositely* (the gating mechanism, now at the free-energy level; `results/figures/fig_pmf_compare.png`):**

| | bare | poly |
|---|---|---|
| F(z) (bulk = 0) | **monotonically downhill to contact** (~−90 kJ/mol) | **barrier**, min ~7.7 Å, ~+100 kJ/mol by 4.3 Å |
| first shell on approach | **desolvates** (Mg–O 6→1, Mg–Cl 6→2) | **shell retained** (Mg–O ≈ Mg–Cl ≈ 6 almost to 4 Å) |

The bare cation **sheds its THF/Cl shell** as it nears Mg(0001) → barrierless favourable approach → it plates. The **POSS network prevents desolvation** (the shell stays locked, CN ≈ 6) → the cation **cannot reach the surface** → a large approach barrier with a preferred ~7.7 Å standoff. This is the **first free-energy-level confirmation of the coordination/transport gating** of §1/§10 (network de-pairs + blocks the Mg face, segregating the carrier), now expressed as a desolvation/approach PMF — and it ties directly to the higher-but-stable poly R_ct in the DRT (§9).

**Honest status — semi-quantitative.** The **shapes, the sign, and the directly-counted CN-desolvation contrast are robust** (CN is counted, not WHAM-processed, and independently explains the PMF); the **magnitudes (~90–107 kJ/mol) are likely overestimated** (moderate sampling, single committee member, chained descent — possible hysteresis). It is the **zero-field, scaled-charge (Mg +1.2), fixed-slab intrinsic** approach free energy — the real overpotential/field lowers the barrier (poly *does* plate, just with higher R_ct, consistent with the DRT). Active-learning rounds + replicate/bidirectional windows are hardening it toward a fully quantitative number; the reduction/plating step itself stays DFT (hybrid).

...
## ★ v3.3 (2026-06-29) — T21d de-pinned wall + whole-box-500K anneal fix; eq verdict: bare A=B SOFT (anion metastable), poly gel-depletion = steric exclusion; field phase running
...
**Bare A=B gate — SOFT (accepted as both-face-average baseline).** The whole-box-hot anneal gave a symmetric *start* (cation 1.00× at the endpoint), but 200 ns of 298 K production **re-diverged**: full 20–200 ns (discard 20) → anion 0.75×, cation 1.50×; tail 100–200 ns → anion 0.70×, cation **1.18× (converging)**. Cation trends toward A=B; **anion is metastably stuck at ~0.70×** (faceB-rich — spontaneous symmetry-breaking, not wall-pinning). The systematic cold-slab pinning is gone; what remains is sparse-ion metastability. Since bare's two faces are equivalent *by construction*, the **both-face-average is the valid bare baseline** (anion 0.043, cation 0.075 /nm² per face); per-face A=B is not cleanly achievable in 200 ns for this sparse-ion finite box.

**Poly eq — gel-depletion signal emerging (steric exclusion).** Tail 100–200 ns: gel-RICH face **flat/depleted** (anion 0.073, cation 0.16 /nm²), gel-POOR face **DRIFTing up** (anion 0.118, cation 0.257) → **1.62× POOR-richer** (both ions). The network **excludes ions from its face** — both anion AND cation → mechanism is **steric exclusion** (gel occupies near-surface volume), NOT a specific anion-repulsion / network-O-coordination. The campaign outcome (anion depleted at the gel-contact anode face) holds via steric exclusion. Network is nearly balanced (RICH/POOR 1.09×), so the label is weak; signal still DRIFTing (not plateaued).

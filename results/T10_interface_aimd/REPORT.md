...
**Poly — interface AIMD (resolved via `aimd_poly`):** the ±1 V field poly run (`bias_prod_poly`, 282-atom) was **abandoned — impractically slow** (~50 min/MD-step on the constant-V Poisson cell → ~10 days for a usable trajectory). The matched poly result uses the **completed zero-field `aimd_poly` trajectory (10,000 steps)**: the Al-anion is **network-sequestered and stays off the front** (T5: anion 4.2× slower, 2× more network-associated; no spontaneous reduction) — the same qualitative outcome as bare (no spontaneous plating). *Field/clean caveat: bare uses the ±1 V field frames, poly the zero-field trajectory; both confirm the rare-event conclusion, and both feed the MLFF dataset as plain-DFT single-points (T16).*

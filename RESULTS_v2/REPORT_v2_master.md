...
| | bare | poly |
|---|---|---|
| Al-anion height above slab | **3.9 ± 0.3 Å** (reaches the electrode) | **8.2 ± 1.0 Å** (held back) |
| stable run | **stable, AL-hardened** (round-2 model) | **full 50 ps, stable** |

**The POSS network holds the reducible Al-anion 2.07× further from the electrode (8.2 vs 3.9 Å)** — the interface-level analogue of the bulk T5 result (anion 2× more network-associated, 4.2× slower) and EPYC's T10 AIMD (~9.5 Å). **The active-learning loop closed end-to-end:** round-1 (`apc_v3_broad`) NaN'd the bare leg at 6.8 ps (Al-anion <5 Å = out-of-distribution vs the T10 AIMD's ~9.5 Å sampling) → **EPYC DFT-labeled the 8 blow-up frames** (`al_queue_bare_t17_labeled.xyz`, slab-masked) → **GPU retrained round-2** (validation still PASSES, force MAE 30.7 meV/Å) → the **bare leg is now stable** (anion 3.9 Å, no blow-up). So the matched bare-vs-poly standoff (2.07×) is now a *robust, AL-hardened* result, not just a qualitative contrast — a complete `uncertainty → label → retrain → harden` cycle of the two-node loop. Full SEI-growth / ToF-SIMS-depth reproduction (plating drive) is the next extension.

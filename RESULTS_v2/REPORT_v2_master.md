...
**The POSS network holds the reducible Al-anion ~1.65× further from the electrode (matched 500 ps r6: `slabMin` 7.57 vs 4.58 Å; honest range 1.4–2.2× as the poly leg is slow-mode-limited, not a runaway)** — the interface-level analogue of the bulk T5 result (anion 2× more network-associated, 4.2× slower) and EPYC's T10 AIMD (~9.5 Å). **The active-learning loop closed end-to-end:** round-1 (`apc_v3_broad`) NaN'd the bare leg at 6.8 ps (Al-anion <5 Å = out-of-distribution vs the T10 AIMD's ~9.5 Å sampling) → **EPYC DFT-labeled the 8 blow-up frames** (`al_queue_bare_t17_labeled.xyz`, slab-masked) → **GPU retrained round-2** (validation still PASSES, force MAE 30.7 meV/Å) → the **bare leg is now stable** (`slabMin` 4.58 Å, no blow-up). Two **independent cross-checks** confirm the bare/poly asymmetry: EPYC's **biased-DFT force-response** (Al responds to ±1 e bias 0.42 vs 0.27 eV/Å = **1.6× stronger in bare**), and the bare **co-deposition AIMD** (the reductive ET is **contact-gated at ~2.5 Å** — bare reaches the front, poly is held ~3 Å beyond it). **Dynamic biased MD — both charge-aware routes attempted and failed (the EDL conclusion rests on the static evidence above, robustly):** (1) the cheap external-`q·E` shortcut was validated vs EPYC's charged frames and **FAILED** (10–30 % capture, blind to the bare/poly asymmetry); (2) a per-charge fine-tune (r6→r6_q−1 on the 72 q=−1 frames) **fits the static charged frames well** (held-out force MAE 61.9 meV/Å ≈ 2.5 %) **but is dynamically unstable** (a localized fine-tune builds runaway force gradients off the training data). A proper dynamic biased MD needs a **charge-conditioned architecture (4G/LES-type MACE) — out of current scope**. This is not a gap in the physics: the **potential-driven EDL modulation is already established by the two static routes** — EPYC's biased-DFT force-response (1.6× stronger in bare) and the co-deposition AIMD (contact-gated reduction at ~2.5 Å). Full SEI-growth / ToF-SIMS-depth reproduction (plating drive) is the next extension.

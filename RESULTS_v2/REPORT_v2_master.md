...
- **Kinetic/transport (T5 — GPU node, done):** in the canonical 4-POSS gel the network **preferentially sequesters the Al-anion** — the anion is **~2× more network-associated than the cation** (47.5 % vs 24.7 % within 0.5 nm; 80 % of anions within 0.7 nm) and **4.2× slower** (D_anion 0.0118 vs bare 0.0499 ×10⁻⁵ cm²/s); **t₊ = 0.50 in both** (matches GITT → rate hypothesis dead). In bare liquid the **Al-anion reaches the Mg(0001) electrode** (contact layer 0.4–0.7 nm); nothing in bare excludes it. The sequestration is **kinetic/structural (lowers the anion's *flux* to the front), not an equilibrium standoff** (~9 Å in both) and is **tortuosity-driven** (Mg–polymer-O direct contact only ~1 %) — **starving the reductive step of anion flux**.
